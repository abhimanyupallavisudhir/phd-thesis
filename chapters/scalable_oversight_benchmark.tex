...
\label{chap:scalable-oversight-benchmark}

\chapterprovenance{Adapted from ``A Benchmark for Scalable Oversight
Mechanisms'' by Abhimanyu Pallavi Sudhir, Jackson Kaunismaa, and Arjun
Panickssery.}

\providecommand{\Protocol}{\texttt{Protocol}}
\providecommand{\Protocolrun}{\texttt{run}}
\providecommand{\Protocolexp}{\texttt{experiment}}
\providecommand{\Agent}{\texttt{Agent}}
\providecommand{\claudenewsonnet}{\texttt{claude-3-5-sonnet-20241022}}
\providecommand{\deepseek}{\texttt{deepseek-v3}}
\providecommand{\gptmini}{\texttt{gpt-4o-mini}}
\providecommand{\deepseekr}{\texttt{deepseek-r1}}
\providecommand{\othree}{\texttt{o3}}
...
For example, in \emph{Debate}, the most widely-known scalable oversight protocol introduced in \citet{irvingAISafetyDebate2018}, the model is incentivized to tell the truth if it knows that a lie can be caught and convincingly refuted by its opponent. A list of other competing proposals is given in \cref{so:sec:related}.
...
One approach, taken by recent works such as \allpreviousworks\footnote{we will collectively refer to these papers as ``\previouswork{}'' or ``\previousworkalt{}'' when making comments that apply to all of them}, is to evaluate these protocols (specifically Debate) \emph{empirically}, by measuring their effect on the accuracy of the ``judge'' (the human or weak model providing feedback).\todo{not sure what to put here vs in related work} %However, these works suffer from two critical limitations: first, they use the ``judge accuracy'' metric to evaluate protocols: as we argue in \cref{sec:asd}, this is not the correct metric from an alignment or safety point-of-view; second, while scalable oversight is designed for settings with a significant \emph{capabilities gap} between the agent (e.g. debater) and the judge, their experiments mainly simulate this capabilities gap with an \emph{information gap}, i.e. where the agent is exposed to information hidden from the judge.%%% leave this commented; it is bad to say they don't look at real capabilities gaps at all, because Kenton's paper is explicitly titled "judging strong LLMs with weak LLMs"
...
We conducted a small experiment to evaluate the following protocols on our benchmark. Configuration details and precise pseudo-Python for each protocol are given in the original paper's supplementary material.
...
We conduct two main types of experiment with these protocols: one, we \emph{benchmark them on ASD} (see~\cref{fig:asds}) to study how well they incentivize truthful behaviour; and two, we study how ASD changes with EAS for each protocol (see~\cref{fig:correlation}), to study how capabilities affect the ability of the protocol to incentivize alignment. All scores are calculated using negative Brier scores for its favourable numerical properties.

\begin{figure}
  \includegraphics[width=\textwidth]{figures/scalable-oversight/asds.png}
  \caption{Average ASD by scalable oversight protocol; the different protocol configurations are described in \cref{sec:results}.} %\texttt{Debate\_t0} is \emph{sequential debate}, where debaters take turns, while \texttt{Debate\_t1} is \emph{simulataneous debate}, where they generate answers simultaneously. In \texttt{Consultancy\_t0} the client starts the conversation while in \texttt{Consultancy\_t1} the consultant does. In all cases, \texttt{n} is the total number of turns.}
  \label{fig:asds}
\end{figure}

% \begin{figure}
%   \includegraphics[width=\textwidth]{figures/scalable-oversight/Debate_t1_n4.png}
%   \caption{Debating with more capable debaters better incentivizes truthfulness.}%\texttt{Debate\_t0} is \emph{sequential debate}, where debaters take turns, while \texttt{Debate\_t1} is \emph{simulataneous debate}, where they generate answers simultaneously. In \texttt{Consultancy\_t0} the client starts the conversation while in \texttt{Consultancy\_t1} the consultant does. In all cases, \texttt{n} is the total number of turns.}
%   \label{fig:asds_ases_sample}
% \end{figure}


\begin{figure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Debate_t0_n2.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Consultancy_t0_n2.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Debate_t1_n2.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Consultancy_t1_n2.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Debate_t0_n4.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Consultancy_t0_n4.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Debate_t1_n4.png}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/scalable-oversight/Consultancy_t1_n4.png}
  \end{subfigure}
...
\begin{enumerate}
    \item \textbf{Debate outperforms all other tested protocols,} both on inducing alignment (\cref{fig:asds}) as well as on the ``combined'' capabilities and alignment measure of Expected Judge Score (\cref{fig:jses}).
...
\begin{figure}
  \includegraphics[width=\textwidth]{figures/scalable-oversight/jses.png}
  \caption{Expected Judge Score (based on propensity to argue) by protocol} %\texttt{Debate\_t0} is \emph{sequential debate}, where debaters take turns, while \texttt{Debate\_t1} is \emph{simulataneous debate}, where they generate answers simultaneously. In \texttt{Consultancy\_t0} the client starts the conversation while in \texttt{Consultancy\_t1} the consultant does. In all cases, \texttt{n} is the total number of turns.}
  \label{fig:jses}
\end{figure}

A summary of results, with ASD, JSE and ASD vs EAS slope values, is given in \cref{tab:results}.

\input{figures/scalable-oversight/results_table}

\section{Future work}

Our main contribution as of this workshop is a benchmark; the experiments themselves are small and demonstrative, conducted with only 100 questions, one judge model (\gptmini{}) and five agent models (\claudenewsonnet{}, and \claudehaiku{} with and without tools, and \deepseek{} without). The results of our preliminary experiments with Debate are promising: apart from benchmarking more protocols, future work should extend our experiments to a wider range of agent models and judge models, as well as with larger numbers of questions to increase statistical significance. Using reasoning models like \deepseekr{} and \othree{} for the agent AI would be particularly novel and valuable.
