...
% Source: /home/user/karmax/infonomy-paper/main.tex (+ app/oldthms.tex),
% lifted essentially verbatim; ACM/AAMAS boilerplate dropped.
% Chapter-local macros: macros/infonomy.tex
% ============================================================

\chapter{Extrapolating volition with recursive information markets}
\label{ch:infonomy}

\chaptersource{This chapter is based on the paper ``Extrapolating Volition with Recursive Information Markets'' by Abhimanyu Pallavi Sudhir and Long Tran-Thanh, which appears at the 8th Games, Agents, and Incentives Workshop (GAIW-26), held as part of the workshops at the 25th International Conference on Autonomous Agents and Multiagent Systems (AAMAS 2026).}
...
\begin{enumerate}
  ...
  % \item \textbf{Mathematical formalism.} We give a fully formal mathematical description for a Bayesian agent participating in the Recursive Information Protocol, as well as a proof of an optimality result \Cref{infonomy:thm:exa} demonstrating that the ``Recursive Inspection Protocol is ex-ante superior to any other admissible protocol'' (where an admissible protocol is one that respects the property rights of the information sellers).
  \item \textbf{Practical implementation.} We provide an implementation of an information market server implementing the Recursive Inspection Protocol, detailed in \Cref{infonomy:sec:practical}, which can directly be applied to various practical applications for information markets such as question-and-answer sites, product inspections and online fact-checking.
\end{enumerate}
...
% among other things this is the subject of scalable oversight \citep{bowmanMeasuringProgressScalable2022}.
% one obv answer is to use markets, which are a measurement device for perferences \citep{Baker_2001}. problem is information asymmetry \citep{lemons} ... buyer's inspection \citep{vanalstyneProposalValuingInformation1999} \citep{Arrow1972}
...
Consider a forecasting decision problem where the agent reports a probability $x\in[0,1]$ for an event $E$, with log-score utility
  ...
  \]
  One concrete joint distribution with these properties is shown in \Cref{infonomy:tab:factcheck-example}.

  \begin{table}[h]
  \centering
  \caption{A distribution exhibiting the fact-checking effect.}
  \label{infonomy:tab:factcheck-example}
  \begin{tabular}{cccc}
  \toprule
...
\label{infonomy:eq:voi-rim}
\end{multline}

These choice sets and utilities are very similar to the decision problems created by the \emph{recursive inspection protocol} described in \Cref{infonomy:alg:rim} and in the main body; except that each action $x^n\in\Actions^n$ is made only consulting the information chosen in $x^{n+1}\in\Actions^{n+1}$. We can then say that the decisions made under this protocol are:
...
\label{infonomy:eq:rim-choices-fin}
\end{equation}
...
\begin{multline}
  ...
  \bigr]
  \label{infonomy:eq:imperfect-x}
\end{multline}

\begin{figure}
  \centering
  \resizebox{\columnwidth}{!}{
  \input{figs/infonomy/imperfect}
  }
...
\begin{theorem}[Recursive Inspections are ex-ante superior to any admissible protocol]
  Let $x^n_*$ be the protocol described in \Cref{infonomy:eq:imperfect-x,infonomy:eq:imperfect-u,infonomy:eq:imperfect-u0}. Then for any admissible purchase protocol $\xi$:
  \begin{multline*}
    \Expectofu[\infooffers^0,\dots\infooffers^{N-1}]{U(\xi(\infooffers))} \\
    \le \Expectofu[\infooffers^0,\dots\infooffers^{N-1}]{U(x_*(\infooffers))}
  \end{multline*}
  \label{infonomy:thm:exa}
\end{theorem}
\begin{proof}
Let $\xi_{*n}$ denote the following admissible protocol:
  ...
  \end{align*}
  Where we have recursively applied \Cref{infonomy:eq:imperfect-u} to transform the expression into one about the value of a node at level $n+1$. Thus the goal is reduced to:
  \begin{multline*}
  ...
  \end{multline*}
  From \Cref{infonomy:eq:imperfect-x} we have that this is true for the expectation over $\infooffers^0,\dots\infooffers^{n}$; we can then simply take the expectation over $\infooffers^{n+1},\dots\infooffers^{N-1}$ and have our result.
\end{proof}

\section{Human feedback for Scalable Oversight}
\label{infonomy:sec:scoring}

% While it is tempting to wonder if the mechanism in \cref{infonomy:app:nonnaive} can directly be use to supply feedback to AI models (i.e. train them to be profitable on a recursive information market), such a mechanism is unsuitable for

\NewDocumentCommand{\allinfo}{}{\mathbf{K}}

The mechanism in \cref{infonomy:app:nonnaive} is unsuitable for settings where the information provided by the sellers must be expensively generated (rather than cheaply retrieving known information) in response to the query---in particular, this is the case with training AI models. Instead, in this setting we may assume that we can initiate as many instances as we want of the AI model we are trying to align: $\beta^1,\beta^2,\dots$ with identical information $\allinfo=\langle K, k, \infty\rangle$. Then let them recursively generate information:
...
\end{itemize}
\label{infonomy:thm:eq}
\end{theorem}

\begin{proof}
We argue by backward induction on subgames.

Fix any history $h^{n-1}=(x^1,\dots,x^{n-1})$. By \Cref{infonomy:def:eq}, player $n$ chooses
\[
x^n_*=\arg\max_{x^n}R^n(H(h^{n-1},x^n,\sigma_*^{n+1},\sigma_*^{n+2},\dots)).
\]
Under \Cref{infonomy:def:mvm}, the null action $\zero$ yields zero marginal contribution and zero price, hence payoff $0$ in that subgame. Therefore a non-null move is chosen at stage $n$ only if it has nonnegative continuation value relative to $\zero$.
...
\end{itemize}
Hence the alternating quantifiers in \Cref{infonomy:def:eq} and in the definition of inextensibility coincide: $[x^1]$ means player $2$ has no profitable continuation in equilibrium from $x^1$.
...
\begin{figure}
  \centering
  \resizebox{\columnwidth}{!}{
  \input{figs/infonomy/rim}
  }
  \caption{The Recursive Inspection Protocol}
  \label{infonomy:fig:rim}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figs/infonomy/infonomy_1.png}
    \caption{Posting a new question (\BuyerContext); viewing \BuyerContexts on the server}
    \label{infonomy:fig:infonomy-1}
  \end{subfigure}

  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figs/infonomy/infonomy_2.png}
    \caption{Posting an answer (\InfoOffer{}); initiating a recursive inspection}
    \label{infonomy:fig:infonomy-2}
  \end{subfigure}

  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figs/infonomy/infonomy_3.png}
    \caption{Inspecting and purchasing \InfoOffers{}; viewing purchased \InfoOffers{}}
    \label{infonomy:fig:infonomy-3}
  \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figs/infonomy/infonomy_4.png}
    \caption{Bot sellers automatically answer recursive \BuyerContexts}
    \label{infonomy:fig:infonomy-4}
  \end{subfigure}

  \caption{Screenshots from the \texttt{infonomy-server} platform}
  \label{infonomy:fig:infonomy}
\end{figure}


A working implementation of an information market server implementing the Recursive Inspection Protocol is available at the \infonomy{} repository\footnote{\infonomyurl} -- some screenshots of the platform's GUI are shown in \Cref{infonomy:fig:infonomy}. Many applications of such a server are immediate:
...
% \Cref{infonomy:alg:rim} itself may be remniscient of scalable oversight proposals such as ``Humans Consulting HCH'' \citeps{christianoHCH}.
...
% ============================================================
% Appendix (paper's app/oldthms.tex). The paper source also has
% app/weiss-rahaman-math.tex, app/naive.tex and app/conitzer.tex,
% which were commented out / not \input in the paper -- left out here.
% ============================================================
\begin{subappendices}

\section{Basic results about value-of-information}
\label{infonomy:sec:thms}

We include a corrected and generalized version of an incorrectly-formulated result in the arXiv version of \citet{weissRedesigningInformationMarkets2024}. These lemmas are perhaps obvious, but the whole theory of value-of-information and information bazaars rests upon them. \Cref{infonomy:lem:voinf} asserts that ``a Bayesian agent expects to gain from information''\footnote{From an AI alignment perspective, this may instead be phrased ``A Bayesian agent trusts a more informed version of itself'', and is a statistical-information version of \emph{Vingean reflection} or \emph{tiling agents} \citep{vingeanReflection}, the desire that (even logically non-omniscient) agents can trust smarter versions of themselves.}. \Cref{infonomy:lem:voins} asserts that ``a Bayesian agent expects to gain from inspection''. Since the agent can always choose not to buy anything after inspecting, in the absence of transaction costs of inspection buyers \emph{will} want to inspect more and more at each step. % that this remains true when the information has a price

\begin{lemma}[Bayesian agent expects to gain from information]
Assume a probability space $(\Samples,\Sigalg,\Prob)$, a set of choices $\Actions$ and a measurable utility function $U:\Samples\times\Actions\to\Reals$. Then for any random variable $I:\Samples\to\Reals$, we have:
\begin{equation*}
\Expectofu[I]{U(\argmax_x\Expectof{U(x)\mid I})} \ge
\max\Expectof{U(x)}
\end{equation*}
\label{infonomy:lem:voinf}
\end{lemma}
\begin{proof}
For all values of $I=i$ and for all $x_0\in\Actions$, we have (from the definition of $\argmax$):
\begin{multline*}
\Expectof{U(\argmax_x \Expectof{U(x)\mid I=i})\mid I=i} \\
\ge \Expectof{U(x_0)\mid I=i}
\end{multline*}
We take the expectation over $i\sim I$ on both sides, apply the law of total expectation and set $x_0=\argmax\Expectof{U(x)}$ to obtain our result.
\end{proof}

\begin{lemma}[Bayesian agent expects to gain from inspection]
Let the recursive construction of $\Actions^n$, $U^n$, $\InfoOffers^n$,$x_*^n$ be as in \Cref{infonomy:sec:bayes}. Then $\forall n$, $\Expectof{U^{n+1}(x^{n+1}_*)}\ge 0$. The same also applies to finite-depth inspections i.e. $\Expectof{U^{n+1}(x^{n+1}_{*:N})}\ge 0$.
\label{infonomy:lem:voins}
\end{lemma}
\begin{proof}
  % \infogood_1^n,\infogood_2^n,\dots, TAGx2
  % ,x^{n+2}_*,\dots TAGx2
\Cref{infonomy:eq:rim-choices} implies that $\Expectof{U^n\left(x^n_*\mid x^{n+1}_*\right)}$ is $\ge$ any version of the same expression with $x^n_*$ replaced by any $x\in\Actions^n$. We choose $x=\zero$, for which the expression evaluates to 0, and take the expectation over $x^{n+1}_*$.
% \begin{align*}
% \Expectof{U^n\left(x^n_*\mid \infogood_1^n,\infogood_2^n,\dots, x^{n+1}_*,x^{n+2}_*,\dots\right)} &\ge \Expectof{U^n\left(x\mid \infogood_1^n,\infogood_2^n,\dots, x^{n+1}_*,x^{n+2}_*,\dots\right)} \forall x\in\Actions^n\\
% &= \\
% \end{align*}
\end{proof}

% \begin{theorem}[Increasing inspection depth improves decisions at all levels]
% Let the recursive construction of $\Actions^n$, $U^n$, $\InfoOffers^n$,$x_*^n$ be as in \Cref{infonomy:sec:bayes}. Then for all $n\le N \le M$:
% \begin{equation*}
% \Expectof{U^n(x^n_{*:N})} \le \Expectof{U^n(x^n_{*:M})}
% \end{equation*}
% \label{infonomy:thm:voins}
% \end{theorem}

% \begin{proof}
% It suffices to prove and transitively apply:
% \begin{equation}
% \Expectof{U^n(x^n_{*:N})} \le \Expectof{U^n(x^n_{*:N+1})}
% \label{infonomy:_eq:ih}
% \end{equation}
% We prove \Cref{infonomy:_eq:ih} by downward induction on $n$ starting at $N$. [...]
% (incomplete proof attempt from the paper source, kept commented out)
% \end{proof}
